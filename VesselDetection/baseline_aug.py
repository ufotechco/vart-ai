...
another_config.read('/content/drive/MyDrive/VesselDetection/config.ini')
base_directory = another_config['DEFAULT']['base_directory']
...
if __name__ == '__main__':
    ...
    train_data = list(zip(sorted(glob(base_directory+'input/training/images/*.tif')),
                          sorted(glob(base_directory+'input/training/1st_manual/*.gif'))))
    ...
    history = model.fit(gen(train_data, au=True), epochs=100, verbose=2,steps_per_epoch= 100*len(train_data)//batch_size,use_multiprocessing=True, workers=16)
    '''warnings.warn(Model.fit_generator is deprecated '''
    #history = model.fit_generator(gen(train_data, au=True), epochs=100, verbose=2,steps_per_epoch= 100*len(train_data)//batch_size,use_multiprocessing=True, workers=16)
